...
<body>
  <div class="container">
    ...
    <div class="control-panel">
        ...
        📩 You can Upload a picture from your device by pressing "Upload" button or "Paste" (Ctrl-V) an image you have in clipboard to send it. (Make sure that the picture is close to the 1:1 ratio to avoid distortion)<br> 
...
<script>
    ...
    //const cols = "%%COLS%%";
    //const rows = "%%ROWS%%";
    //const ipAddress = "192.168.178.77";
    const cols = 43;
    const rows = 43;
    ...
    canvas.addEventListener('mousedown', e => { saveState(); isDrawing = true; drawPixel(e); });
    canvas.addEventListener('mousemove', e => { if (isDrawing) drawPixel(e); });
    canvas.addEventListener('mouseup', () => { isDrawing = false; sendDrawBuffer(); });
    canvas.addEventListener('mouseleave', () => { isDrawing = false; sendDrawBuffer(); });
